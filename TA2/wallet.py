...
class Wallet():
    ...
    # SA2: Get currencyType parameter    
    def getBalance(self, address, currencyType):
        balance = web3.eth.get_balance(address)
        # SA2: check if currency type is dollar
        if(currencyType=='dollar'):
            # SA2: Set the conversionRate to 1826
            conversionRate = 1826            
            # SA2: Get new balance byt multiplying the balance and currencyRate
    ...
    # SA2: Add currencyType parameter
    def getTransactions(self, address, currencyType):
        ...
        # SA2: Check if currencyType is dollar
        if(currencyType == 'dollar'):
            # SA2: Set the conversionRate to 1826
            conversionRate = 1826     
            # SA2: Convert all entries in the amount field to Dollar  
            allUserTnx = list(map(lambda tnx: {**tnx, "amount": float(tnx["amount"]) * conversionRate}, allUserTnx))
